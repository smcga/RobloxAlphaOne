...
- Adds a 100-rank secondary progression track (`Age 00 Newborn Spark` through `Age 99 Timeless Paragon`) based on score thresholds, with configurable age-themed rank names in shared constants.
...
- Grants each rank a server-driven aura that gets more extravagant over time (richer colors, denser particles, and brighter glow at higher ranks).
...
8. Verify expected MVP behavior:
   ...
   - You see a rank progression panel (`Age 00 Newborn Spark` initially) with a filling progress bar and progress text toward the next rank.
   ...
   - Your character aura updates each time you unlock a new rank, with progressively more dramatic visuals.
