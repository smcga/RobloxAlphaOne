...
- Spawns players into a stylized starter world with a baseplate, hills, paths, and Creator Store-inspired tree/rock/bush/flower props built from deterministic decor specs.
...
5. Verify expected MVP behavior:
   - A `Baseplate`, a `WorldDecor` folder (hills/paths/multiple trees/rocks/bushes/flower patches), blue `ScoreZone`, red `HighScoreZone`, orange `1700'sZone`, and magenta `RebirthZone` appear in `Workspace`.
