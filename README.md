...
- Shows HUD scoring-state feedback (`Scoring: active/idle`) and emits score sparkles from the player while they are actively gaining score in an unlocked age zone.
...
8. Verify expected MVP behavior:
   ...
   - While score is actively ticking, the HUD shows `Scoring: active` and score particles sparkle off your character; leaving active zones switches to `Scoring: idle` and stops the particle effect.
